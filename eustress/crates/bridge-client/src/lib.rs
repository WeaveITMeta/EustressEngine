...
//! `eustress-headless`) binds a localhost TCP listener (`127.0.0.1:<port>`)
...
//! On unix platforms it *also* binds a Unix domain socket, advertised via a
//! pointer file at `<universe>/.eustress/engine.sock` (see
//! `eustress_engine::engine_bridge::unix_socket_file` for why that file's
//! contents are a path rather than the socket itself — `AF_UNIX` addresses
//! are capped at ~104-108 bytes and Universe paths routinely exceed that).
//! Sibling processes (the MCP server, the `eustress` CLI, future plugins)
//! discover the bridge by reading those files, preferring the socket when
//! present (see [`connect_under`]).
//!
//! ## Why two transports
//!
...
//! the same host and the engine is definitely listening. `AF_UNIX` sockets
//! are filesystem objects rather than network connections, so they fall
//! outside that policy.
//!
...
//! `std::os::unix::net::UnixStream`): every consumer's call site is a
//! single short request/response round-trip, so pulling in an async
//! runtime here would buy nothing and would make this crate a heavier add
//! for the CLI. Every failure path returns a friendly `Err(String)` — this
//! function never panics, so a caller can surface "engine isn't running"
//! cleanly (to an AI, to a terminal, wherever).
...
/// services bridge requests on its main frame (or, headless, its
/// `ScheduleRunner` tick), so a live process answers in well under a
/// frame; 2 s is generous headroom that still fails fast when nothing is
/// listening.
...
/// are the real (short, temp-dir) bind path. `AF_UNIX` addresses are capped
/// at ~104-108 bytes, and Universe paths routinely exceed that once nested
/// a few directories deep, so the socket can't live at `<dir>/.eustress/`
/// directly.
    ...
    #[cfg(unix)]
    {
        ...
        if let Ok(raw) = std::fs::read_to_string(&pointer_path) {
            ...
            if !bind_path.is_empty() {
                if let Ok(s) = UnixStream::connect(bind_path) {
                    return Ok(BridgeStream::Unix(s));
                }
                // Stale pointer (engine crashed, or nothing bound at that
                // path) — fall through to TCP rather than failing outright.
            }
        }
    }
...
pub fn call_engine(universe_dir: &Path, method: &str, params: Value) -> Result<Value, String> {
    ...
    // writes both there too). This lets a caller find the live engine even
    // when it launched into a DIFFERENT universe than the one it's
    // configured for.
...
mod tests {
    ...
    fn call_engine_round_trips_over_unix_socket() {
        ...
        // exactly what `unix_socket_file::bind_path_for` produces in the
        // engine, just without depending on engine-crate code from this
        // bridge-client-crate test.
        let bind_path =
