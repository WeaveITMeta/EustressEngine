...
/// Holds all staged file changes awaiting user approval. Derives
/// `Resource` only when the `bevy` feature is on — the engine enables
/// it and inserts this as a Bevy resource; the MCP server skips the
/// feature and uses the same struct as a plain value (it doesn't host
/// a Bevy world).
