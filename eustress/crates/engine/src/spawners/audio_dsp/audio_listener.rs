//! `AudioListenerSpawner` — `ClassSpawner` for [`ClassName::AudioListener`].
//!
//! Per `docs/architecture/CLASS_REGISTRY.md` §8 (Wave 7.E). Config-attach
//! spatial audio listener node; the DSP graph wiring is deferred.
...
use eustress_common::classes::{AudioListener, ClassName, PropertyValue};
use eustress_common::{Attributes, Tags};

use super::{apply_metadata_edit, export_metadata, import_metadata, instance_from_bag};

/// Zero-sized spawner for [`ClassName::AudioListener`].
#[derive(Default)]
pub struct AudioListenerSpawner;

impl ClassSpawner for AudioListenerSpawner {
    fn class_name(&self) -> ClassName {
        ClassName::AudioListener
    }

    fn spawn(&self, ctx: &mut SpawnCtx, props: &PropertyBag) -> Entity {
        let instance = instance_from_bag(ClassName::AudioListener, props);
        let name = instance.name.clone();
        let d = AudioListener::default();
        let comp = AudioListener {
    ...
    fn apply_edit(&self, world: &mut World, entity: Entity, props: &PropertyBag) -> bool {
        ...
        if let Some(v) = props.get_string("audio_interaction_group") {
            if let Some(mut comp) = world.get_mut::<AudioListener>(entity) {
    ...
    fn export_to_toml(&self, world: &World, entity: Entity) -> toml::Value {
        ...
        root.insert(
            "metadata".to_string(),
            toml::Value::Table(export_metadata(world, entity, "AudioListener")),
        );
        if let Some(comp) = world.get::<AudioListener>(entity) {
...
mod tests {
    ...
    #[test]
    fn class_name_is_audio_listener() {
        assert_eq!(AudioListenerSpawner.class_name(), ClassName::AudioListener);
    }

    #[test]
    fn lod_bundle_is_empty_at_every_tier() {
        for tier in [LodTier::Hero, LodTier::Active, LodTier::Streamed, LodTier::Horizon] {
            assert!(AudioListenerSpawner.lod_components(tier).is_empty());
        }
    }
    ...
    fn import_from_toml_reads_group() {
        ...
        let bag = AudioListenerSpawner.import_from_toml(&value);
