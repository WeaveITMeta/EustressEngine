//! `AudioChorusSpawner` — `ClassSpawner` for [`ClassName::AudioChorus`].
//!
...
use eustress_common::classes::{ClassName, PropertyValue, AudioChorus};
use eustress_common::{Attributes, Tags};

use super::{apply_metadata_edit, export_metadata, import_metadata, instance_from_bag};

/// Zero-sized spawner for [`ClassName::AudioChorus`].
#[derive(Default)]
pub struct AudioChorusSpawner;

impl ClassSpawner for AudioChorusSpawner {
    fn class_name(&self) -> ClassName {
        ClassName::AudioChorus
    }

    fn spawn(&self, ctx: &mut SpawnCtx, props: &PropertyBag) -> Entity {
        let instance = instance_from_bag(ClassName::AudioChorus, props);
        let name = instance.name.clone();
        let d = AudioChorus::default();
        let comp = AudioChorus {
    ...
    fn apply_edit(&self, world: &mut World, entity: Entity, props: &PropertyBag) -> bool {
        ...
        if let Some(mut comp) = world.get_mut::<AudioChorus>(entity) {
    ...
    fn export_to_toml(&self, world: &World, entity: Entity) -> toml::Value {
        ...
        root.insert(
            "metadata".to_string(),
            toml::Value::Table(export_metadata(world, entity, "AudioChorus")),
        );
        if let Some(comp) = world.get::<AudioChorus>(entity) {
...
mod tests {
    ...
    fn class_name_matches() {
        assert_eq!(AudioChorusSpawner.class_name(), ClassName::AudioChorus);
    }

    #[test]
    fn lod_bundle_is_empty_at_every_tier() {
        for tier in [LodTier::Hero, LodTier::Active, LodTier::Streamed, LodTier::Horizon] {
            assert!(AudioChorusSpawner.lod_components(tier).is_empty());
        }
    }

    #[test]
    fn import_from_toml_reads_first_field() {
        let toml_src = "[metadata]\nclass_name = \"AudioChorus\"\nname = \"X\"\n[properties]\nrate= 0.5\n";
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let bag = AudioChorusSpawner.import_from_toml(&value);
