//! `EqualizerSoundEffectSpawner` — `ClassSpawner` for [`ClassName::EqualizerSoundEffect`].
//!
...
use eustress_common::classes::{ClassName, PropertyValue, EqualizerSoundEffect};
use eustress_common::{Attributes, Tags};

use super::{apply_metadata_edit, export_metadata, import_metadata, instance_from_bag};

/// Zero-sized spawner for [`ClassName::EqualizerSoundEffect`].
#[derive(Default)]
pub struct EqualizerSoundEffectSpawner;

impl ClassSpawner for EqualizerSoundEffectSpawner {
    fn class_name(&self) -> ClassName {
        ClassName::EqualizerSoundEffect
    }

    fn spawn(&self, ctx: &mut SpawnCtx, props: &PropertyBag) -> Entity {
        let instance = instance_from_bag(ClassName::EqualizerSoundEffect, props);
        let name = instance.name.clone();
        let d = EqualizerSoundEffect::default();
        let comp = EqualizerSoundEffect {
    ...
    fn apply_edit(&self, world: &mut World, entity: Entity, props: &PropertyBag) -> bool {
        ...
        if let Some(mut comp) = world.get_mut::<EqualizerSoundEffect>(entity) {
    ...
    fn export_to_toml(&self, world: &World, entity: Entity) -> toml::Value {
        ...
        root.insert(
            "metadata".to_string(),
            toml::Value::Table(export_metadata(world, entity, "EqualizerSoundEffect")),
        );
        if let Some(comp) = world.get::<EqualizerSoundEffect>(entity) {
...
mod tests {
    ...
    fn class_name_matches() {
        assert_eq!(EqualizerSoundEffectSpawner.class_name(), ClassName::EqualizerSoundEffect);
    }

    #[test]
    fn lod_bundle_is_empty_at_every_tier() {
        for tier in [LodTier::Hero, LodTier::Active, LodTier::Streamed, LodTier::Horizon] {
            assert!(EqualizerSoundEffectSpawner.lod_components(tier).is_empty());
        }
    }

    #[test]
    fn import_from_toml_reads_first_field() {
        let toml_src = "[metadata]\nclass_name = \"EqualizerSoundEffect\"\nname = \"X\"\n[properties]\nlow_gain= 0.5\n";
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let bag = EqualizerSoundEffectSpawner.import_from_toml(&value);
