//! `CompressorSoundEffectSpawner` — `ClassSpawner` for [`ClassName::CompressorSoundEffect`].
//!
...
use eustress_common::classes::{ClassName, PropertyValue, CompressorSoundEffect};
use eustress_common::{Attributes, Tags};

use super::{apply_metadata_edit, export_metadata, import_metadata, instance_from_bag};

/// Zero-sized spawner for [`ClassName::CompressorSoundEffect`].
#[derive(Default)]
pub struct CompressorSoundEffectSpawner;

impl ClassSpawner for CompressorSoundEffectSpawner {
    fn class_name(&self) -> ClassName {
        ClassName::CompressorSoundEffect
    }

    fn spawn(&self, ctx: &mut SpawnCtx, props: &PropertyBag) -> Entity {
        let instance = instance_from_bag(ClassName::CompressorSoundEffect, props);
        let name = instance.name.clone();
        let d = CompressorSoundEffect::default();
        let comp = CompressorSoundEffect {
    ...
    fn apply_edit(&self, world: &mut World, entity: Entity, props: &PropertyBag) -> bool {
        ...
        if let Some(mut comp) = world.get_mut::<CompressorSoundEffect>(entity) {
    ...
    fn export_to_toml(&self, world: &World, entity: Entity) -> toml::Value {
        ...
        root.insert(
            "metadata".to_string(),
            toml::Value::Table(export_metadata(world, entity, "CompressorSoundEffect")),
        );
        if let Some(comp) = world.get::<CompressorSoundEffect>(entity) {
...
mod tests {
    ...
    fn class_name_matches() {
        assert_eq!(CompressorSoundEffectSpawner.class_name(), ClassName::CompressorSoundEffect);
    }

    #[test]
    fn lod_bundle_is_empty_at_every_tier() {
        for tier in [LodTier::Hero, LodTier::Active, LodTier::Streamed, LodTier::Horizon] {
            assert!(CompressorSoundEffectSpawner.lod_components(tier).is_empty());
        }
    }

    #[test]
    fn import_from_toml_reads_first_field() {
        let toml_src = "[metadata]\nclass_name = \"CompressorSoundEffect\"\nname = \"X\"\n[properties]\nattack= 0.5\n";
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let bag = CompressorSoundEffectSpawner.import_from_toml(&value);
