//! `ChorusSoundEffectSpawner` — `ClassSpawner` for [`ClassName::ChorusSoundEffect`].
//!
...
use eustress_common::classes::{ClassName, PropertyValue, ChorusSoundEffect};
use eustress_common::{Attributes, Tags};

use super::{apply_metadata_edit, export_metadata, import_metadata, instance_from_bag};

/// Zero-sized spawner for [`ClassName::ChorusSoundEffect`].
#[derive(Default)]
pub struct ChorusSoundEffectSpawner;

impl ClassSpawner for ChorusSoundEffectSpawner {
    fn class_name(&self) -> ClassName {
        ClassName::ChorusSoundEffect
    }

    fn spawn(&self, ctx: &mut SpawnCtx, props: &PropertyBag) -> Entity {
        let instance = instance_from_bag(ClassName::ChorusSoundEffect, props);
        let name = instance.name.clone();
        let d = ChorusSoundEffect::default();
        let comp = ChorusSoundEffect {
    ...
    fn apply_edit(&self, world: &mut World, entity: Entity, props: &PropertyBag) -> bool {
        ...
        if let Some(mut comp) = world.get_mut::<ChorusSoundEffect>(entity) {
    ...
    fn export_to_toml(&self, world: &World, entity: Entity) -> toml::Value {
        ...
        root.insert(
            "metadata".to_string(),
            toml::Value::Table(export_metadata(world, entity, "ChorusSoundEffect")),
        );
        if let Some(comp) = world.get::<ChorusSoundEffect>(entity) {
...
mod tests {
    ...
    fn class_name_matches() {
        assert_eq!(ChorusSoundEffectSpawner.class_name(), ClassName::ChorusSoundEffect);
    }

    #[test]
    fn lod_bundle_is_empty_at_every_tier() {
        for tier in [LodTier::Hero, LodTier::Active, LodTier::Streamed, LodTier::Horizon] {
            assert!(ChorusSoundEffectSpawner.lod_components(tier).is_empty());
        }
    }

    #[test]
    fn import_from_toml_reads_first_field() {
        let toml_src = "[metadata]\nclass_name = \"ChorusSoundEffect\"\nname = \"X\"\n[properties]\nrate= 0.5\n";
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let bag = ChorusSoundEffectSpawner.import_from_toml(&value);
