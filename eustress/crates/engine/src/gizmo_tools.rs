...
fn diagnose_gizmos_once(
    ...
    info!("  DefaultGizmoGroup:   enabled={}, line_width={}, depth_bias={}",
        cfg.enabled, cfg.line.width, cfg.depth_bias);
    let (cfg, _) = config_store.config::<bevy::light::gizmos::LightGizmoConfigGroup>();
...
fn configure_gizmos(mut config_store: ResMut<GizmoConfigStore>) {
    ...
    {
        let (config, light_config) = config_store.config_mut::<bevy::light::gizmos::LightGizmoConfigGroup>();
        config.enabled = true;
        ...
        light_config.color = bevy::light::gizmos::LightGizmoColor::MatchLightColor;
    }
}
